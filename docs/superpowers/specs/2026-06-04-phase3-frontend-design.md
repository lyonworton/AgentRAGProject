...
| 生产化 | MinIO 对象存储 + Prometheus/Grafana 监控 + Nginx 部署 | P1 |
...
| 路由 | React Router v7 (route-based lazy loading) |
| 服务端状态 | TanStack Query v5 (缓存 / 重试 / 乐观更新) |
| 客户端状态 | React Context (认证信息、UI 偏好) |
| 流式通信 | fetch + ReadableStream (SSE 解析) |
| 前端部署 | Nginx 静态文件 + 反向代理到 FastAPI |
| 对象存储 | MinIO (S3-compatible, 文件持久化) |
...
```
React Component → TanStack Query hook → fetch(/api/v1/...) → FastAPI
                                                                  │
...
async function request<T>(path: string, opts?: RequestInit): Promise<T> {
  const token = getAuthToken(); // from React Context or localStorage
...
// api/queries.ts
interface SSEEvent {
  event: string;    // "status" | "chunk" | "citation" | "done"
  data: string;     // JSON payload
}

async function* streamQuery(query: string, opts?: QueryOptions): AsyncGenerator<SSEEvent> {
...
- **用途**: 上传文件持久化 (替换本地文件系统) + Milvus 数据备份
- **Docker**: `minio/minio:latest`, 端口 9000 (API) + 9001 (Console)
- **集成**: FastAPI 通过 `minio` Python SDK 读写 bucket
- **前端**: 文件上传走 FastAPI → MinIO, 前端不直连

### 6.2 Prometheus + Grafana

**指标**:
- HTTP: 请求速率 / 错误率 / 延迟分位数 (p50/p95/p99)
- Agent: 迭代次数分布 / quality_score 分布 / 每查询 token 用量
- 存储: Milvus/Neo4j/ES 健康状态 (up/down)
- 成本: LLM daily cost 累计

**Grafana Dashboards**: API 概览面板 + Agent 详细面板 (各 1 个)

**告警**: P1 (日费用超预算 1.5x) / P2 (摄入失败率 >10%) — 日志报警，MVP 不做 webhook

### 6.3 Docker Compose 新增
...
| 层 | 内容 | 工具 |
|----|------|------|
| 前端单元 | 组件渲染、hooks 逻辑、API mock | Vitest + React Testing Library + MSW |
| 前端 E2E | Admin CRUD 流程 + Chat 多轮对话 | Playwright |
| 基础设施 | MinIO 上传/下载 | pytest + testcontainers |
...
| TanStack Query | 缓存/重试/乐观更新开箱即用, 代替手写 useEffect |
| Nginx 静分 + 反向代理 | 无需 Node.js 服务端, 静态文件直出 |
| MinIO 替代本地文件系统 | S3 兼容 + 多实例共享 + 后续可迁移云端 |
| 不拆分 frontend/admin 和 frontend/chat | React.lazy + 路由分块 足够隔离, 需要时可拆 |